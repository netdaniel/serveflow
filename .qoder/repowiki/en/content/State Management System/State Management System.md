...
## Core Components
- StoreProvider: Wraps the app with React Context, initializes Supabase auth, loads organization and profile data, and exposes a comprehensive set of actions for CRUD operations on groups, roles, volunteers, events, and assignments. It also provides a refreshData function to reload all data.
- useStore: A custom hook that returns the current store value for consumption by components.
...
- Data loading: parallel fetch of groups, roles, volunteers, events, and assignments; transforms volunteer roles for compatibility.
...
The store is a single source of truth for all application data. Components subscribe to the store via useStore and receive both state and action functions. Data is loaded from Supabase and kept in sync through:
- Auth state subscription to keep session and derived profile/organization in sync.
- Parallel data loading on profile change to populate groups, roles, volunteers, events, and assignments.
...
Provider->>DB : parallel load groups, roles, volunteers, events, assignments
DB-->>Provider : { data[], error? } for each
Provider->>Comp : provide store value via context
Comp->>Provider : call action (add/update/delete)
Provider->>DB : mutate operation
DB-->>Provider : { data?, error? }
Provider->>DB : refresh data (loadAllData)
DB-->>Provider : { data[], error? }
Provider->>Comp : update state (re-render)
```

**Diagram sources**
- [store.jsx](file://src/services/store.jsx#L21-L111)
- [store.jsx](file://src/services/store.jsx#L114-L422)
- [store.jsx](file://src/services/store.jsx#L432-L460)

## Detailed Component Analysis

### StoreProvider and useStore
- Context: A single context holds the store value, including user, profile, organization, loading flag, and all actions.
...
LoadProfile --> ParallelLoad["Parallel load groups, roles, volunteers, events, assignments"]
ParallelLoad --> Ready["Store ready"]
```
...
- useStore: Returns the store value from context. Components can destructure the parts they need (e.g., user, volunteers, events, roles, addVolunteer, updateEvent).
- Example usage:
  - Dashboard consumes user and lists counts for volunteers, events, and roles.
  - Volunteers page consumes volunteers, roles, groups, and CRUD actions.
  - Schedule page consumes events, assignments, roles, volunteers, groups, and assignment-related actions.
  - Roles page consumes roles and groups, and manages both roles and groups.

```mermaid
flowchart TD
Hook["useStore()"] --> Context["Context Value"]
...
- [Dashboard.jsx](file://src/pages/Dashboard.jsx#L22-L28)
- [Volunteers.jsx](file://src/pages/Volunteers.jsx#L8-L8)
- [Schedule.jsx](file://src/pages/Schedule.jsx#L8-L8)
- [Roles.jsx](file://src/pages/Roles.jsx#L7-L7)
...
- Loading pattern:
  ...
  - Parallel loading of all entities when profile is ready.
...
- [store.jsx](file://src/services/store.jsx#L90-L94)
...
- Layout and navigation: Layout checks user presence and redirects to landing if not authenticated.
- Pages:
  - Dashboard: Reads user and entity counts.
  - Volunteers: Manages volunteer CRUD, CSV import, and role selection.
  - Schedule: Manages events, assignments, and email sharing.
  - Roles: Manages groups and roles with modal dialogs.
- Modal: Reusable dialog component used across pages.
...
- [Schedule.jsx](file://src/pages/Schedule.jsx#L511-L729)
- [Modal.jsx](file://src/components/Modal.jsx#L1-L50)
...
- Internal dependencies:
  ...
  - All pages depend on store.jsx via useStore.
  - Layout and AuthLayout wrap pages and manage navigation.
...
- Data loading:
  ...
  - Parallel load of groups, roles, volunteers, events, assignments.
- Mutations:
